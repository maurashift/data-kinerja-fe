...
import { getCookie } from "@/src/lib/cookie"; // Pastikan path ini benar
...
const AddDataTableModal = ({
  ...
  const fetchJenisData = async () => {
    ...
    // // Ambil kode OPD dari cookie untuk filter list jenis data
    // const dinasCookie = safeParseOption(getCookie("selectedDinas"));
    // const kodeOpd = dinasCookie?.value;

    // if (!kodeOpd) return;

    setIsLoadingJenisData(true);
    try {
      // URL: Menggunakan list per OPD tanpa /alur-kerja
      ...
      // preselect dari prop jika ada ID yang cocok
  ...
  }, [isOpen, branding?.api_perencanaan]);
  ...
  const onSubmit: SubmitHandler<FormValue> = async (data) => {
    ...
    const payload = {
      ...
      //kode_opd: dinasCookie.value, // Tambahkan kode_opd
    ...
    try {
      // URL: Menggunakan datakinerjaopd tanpa /alur-kerja
      const response = await fetch(
