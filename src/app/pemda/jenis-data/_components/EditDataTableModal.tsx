      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-4">
                  ...
                  {fields.map((row, idx) => (
                    <tr key={row.id}>
                      <td className="border p-2 text-center">
                        <input className="w-full bg-gray-100 p-2 rounded text-center cursor-not-allowed border-none focus:ring-0" value={row.tahun} readOnly />
                      </td>
          ...
          <div className="flex flex-col gap-3 mt-6">
            <button type="submit" disabled={isSubmitting} className="w-full py-3 rounded-lg text-white font-bold bg-green-600 hover:bg-green-700 disabled:opacity-50 transition-colors">
              {isSubmitting ? "Menyimpan..." : "Simpan Perubahan"}
            </button>
            <button type="button" onClick={handleClose} className="w-full py-3 rounded-lg text-white font-bold bg-red-500 hover:bg-red-600 transition-colors">
              Batal
            </button>
          </div>
        </form>
...
const InputField = ({ control, name, label, error, isTextarea = false }: any) => (
  <div>
    ...
    <Controller
      ...
      render={({ field }) =>
        isTextarea ? (
          <textarea {...field} value={field.value || ""} className={`w-full p-3 border rounded-md focus:ring-2 focus:ring-blue-500 outline-none transition-shadow ${error ? 'border-red-500' : 'border-gray-300'}`} />
        ) : (
          <input {...field} value={field.value || ""} className={`w-full p-3 border rounded-md focus:ring-2 focus:ring-blue-500 outline-none transition-shadow ${error ? 'border-red-500' : 'border-gray-300'}`} />
        )
      }
    />
